...
def confirmAdd():
    ...
    if authorName.get() == "" or bookName.get() == "":
        ...
        sqlcursor.execute(
            f"insert into all_books values ({latest + 1}, '{bookName.get()}', '{authorName.get()}',{Genre.get()})"
        )
        sqldatabase.commit()

        sqlcursor.execute(
            f"insert into available_books values ({latest + 1}, '{bookName.get()}', '{authorName.get()}',{Genre.get()})"
        )
        sqldatabase.commit()
